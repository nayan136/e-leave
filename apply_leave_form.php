...
<body>
	...
	<div class="ui container">
		...
		<?php
			...
			function application_report($date,$user_id,$connection){
				$query = "SELECT id FROM history WHERE user_id='{$user_id}' AND from_date='{$date}'";
			...
			if(isset($_POST["apply_form_halfday"])){
				if(!empty($_POST["half_day_date"]) && !empty($_POST["reason"])){
					...
					if($duplicate_found >0){
						...
						$check_duplicate_shift = "SELECT * FROM history WHERE user_id='{$user_id}' AND
					...
					if($duplicate_found == 0){
						...
						if (mysqli_affected_rows($connection)>=0) {
							...
							$data = application_report($from_date_for_half_day, $user_id,$connection);
    ...
    // 				}
				...
				if(!empty($from_date) && !empty($to_date) && !empty($reason)){
					...
					if($date_diff <5){
						$query = "SELECT cl_left FROM user WHERE id='{$user_id}'";
						...
						if($days_left > $date_diff){
							...
							$no_duplicate = true;
							foreach($period as $date){
								$check_duplicate = "SELECT id FROM history WHERE user_id='{$user_id}'AND from_date='{$date}'";
								$check_duplicate_search = mysqli_query($connection, $check_duplicate);
								$duplicate_found = mysqli_num_rows($check_duplicate_search);
								if($duplicate_found > 0){
									$no_duplicate = false;
	...
	<div class="sixteen wide column">
		<h2 class="center_text">Application Form</h2>
			...
			<div class="ui bottom attached tab segment active" data-tab="first">
			   		<form class="ui form segment" action="apply_leave_form.php" method="post">
